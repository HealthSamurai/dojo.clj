...
(comment
  ...
  (def f3 (future (.join (.start node3))))

  (future (p/deliver node-p3 (.join (.start node3))))


  (def distrib-map (.build (.withCacheEnabled (.mapBuilder node3 "my-map"))))
